...
public class LikeController {
    ...
    @PostMapping("/like")
    @ResponseBody
    public Long like(Integer carId,HttpServletRequest request){
        //从请求头部获得token
        String token = request.getHeader("token");//get token
        Integer userId = TokenUtil.parseToken(token).getUserId();
        //在likeKey对应的集合中加入当前用户的id
        long likeCount = likeService.like(userId,carId);
    ...
    @PostMapping("/dislike")
    @ResponseBody
    public long disLike(Integer carId,HttpServletRequest request){
        //从请求头部获得token
        String token = request.getHeader("token");//get token
        Integer userId = TokenUtil.parseToken(token).getUserId();
        long likeCount = likeService.disLike(userId,carId);
        if (likeCount <= 0){
            likeCount = 0;
        }
        return likeCount;
